...
export default function Create({ open, onCancel }) {
  ...
  const handleSubmit = (values) => {
    ...
    // Prepare data with organizationId from form selection
    const payload = {
      ...values,
    };
  ...
  // Prepare organization options from API response
  const organizationOptions =
    organizationData?.data?.map((org) => ({
      value: org.id,
      label: `${org.orgCode} - ${org.name}`,
    })) || [];
