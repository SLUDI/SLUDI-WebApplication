...
import { changeImageUrl } from "../../services/licenseIssued/licenseIssued";
...
export default function LicenseIssuanceForm() {
  ...
  // Fetch profile photo from API using CID
  useEffect(() => {
    const fetchProfilePhoto = async () => {
      ...
      try {
        const base64Photo = await changeImageUrl(photoCid);
        if (base64Photo) {
          setProfilePhotoBase64(base64Photo);
        }
      } catch (error) {
        console.error("Failed to fetch profile photo:", error);
        notifyError("Failed to load profile photo");
      } finally {
  ...
  // Format photo URL
  const getPhotoUrl = () => {
    if (!profilePhotoBase64) return null;
    ...
    return `data:image/jpeg;base64,${profilePhotoBase64.data}`;
  };
                ...
                <Card style={{ marginTop: 16 }}>
                  <Row justify="space-between" align="middle">
                    <Space>
                      ...
                      <Text>
                        All information will be recorded on blockchain
                      </Text>
                    </Space>
